...
async def search_and_enrich_scopus_endpoint(request: Request):
    """
    Descarga registros desde Scopus API, extrae DOIs y los cruza en ClickHouse (works_flat)
    ...
    # 1. Paginación de registros en Scopus API
    scopus_entries = []
    total_scopus = 0
    start = 0
    page_size = 25  # Elsevier estándar

    try:
        while start < max_results:
            fetch_url = f"https://api.elsevier.com/content/search/scopus?query={urllib.parse.quote(query)}&count={page_size}&start={start}"
            res = requests.get(fetch_url, headers=headers, timeout=20)
            if res.status_code != 200:
                logger.error(f"Error Scopus API en start={start}: {res.status_code}")
                break
            ...
            if total_scopus == 0:
                ...
                total_scopus = int(total_str) if total_str.isdigit() else 0

            entries = search_results.get("entry", [])
            if not entries or (isinstance(entries, list) and len(entries) == 1 and entries[0].get("error")):
                break

            scopus_entries.extend(entries)
            start += len(entries)

            if len(entries) < page_size or start >= total_scopus:
                break
    except Exception as e:
    ...
    # Formateamos variantes con 'https://doi.org/' y bare DOI
